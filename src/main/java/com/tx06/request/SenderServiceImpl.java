...
public class SenderServiceImpl {
    ...
    public void send(Apidoc apidoc){
        try {
            ...
            String str = HttpUtil.post(SpringUtil.getBean(Prop.class).getServer().getBasePath() + "/apidoc/add?version="+VERSION,param);
    ...
    public void rsycnFieldComment(List<Map<String,Object>> list){
        try {
            ...
            String str = HttpUtil.post(SpringUtil.getBean(Prop.class).getServer().getBasePath() + "/apidoc/rsycnFieldComment?version="+VERSION,param);
            JSONObject rs = JSON.parseObject(str);
            if(!rs.containsKey("code") || rs.getInteger("code").intValue()!=200){
                log.error("apidoc 接口请求失败：" + rs.getString("msg"));
            }
    ...
    public void rsyncDict(List<Map<String,Object>> list) {
        try {
            ...
            String str = HttpUtil.post(SpringUtil.getBean(Prop.class).getServer().getBasePath() + "/apidoc/rsyncDict?version="+VERSION,param);
            JSONObject rs = JSON.parseObject(str);
            if(!rs.containsKey("code") || rs.getInteger("code").intValue()!=200){
                log.error("apidoc 接口请求失败：" + rs.getString("msg"));
            }
